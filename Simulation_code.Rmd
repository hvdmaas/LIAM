...
gt <- 0 # global external field (only used in sim 5)
```
...
Pr=function(n,w,t,beta)
{
  Pi=rep(0,2^n)
  for(i in 1:2^n)
    Pi[i]=exp(-beta*ham(bm[i,],n,t+gt,w))
  sumPi=sum(Pi)
  for(i in 1:2^n)
    Pi[i]=Pi[i]/sumPi
  Pi
...
for(sim in 1:2){
  ...
  for(j in 1:m){
    ...
    p=1/(1+exp(beta*(ham(x2,n,t+gt,w)-ham(x,n,t+gt,w)))) # metropolis
...
for(sim in 1:2)
  ...
  for (j in 1:m)
    ...
    x2=x;x2[i]=x2[i]*-1
    p=1/(1+exp(beta[j]*(ham(x2,n,t+gt,w)-ham(x,n,t+gt,w))))  # metropolis
    if(runif(1)<p) x=x2 # update state
    dat[j,]=x
...
- dispositions are all adjusted with -0.35 in the moderately persuasive condition (left panels), or with -0.55 in the strong persuasion condition (right panels)
...
for(sim in 1:4){ 
  ...
  if(sim==1){
    ...
    statesIP1TMinus.55 <- allStates
    endstatesIP1TMinus.55 <- endStates
  } else if (sim == 3){
    statesIP3TMinus.35 <- allStates
    endstatesIP3TMinus.35 <- endStates
  } else if (sim==4){
    statesIP3TMinus.55 <- allStates
    endstatesIP3TMinus.55 <- endStates
  }

}
####

Sim5Results <- list(statesIP1TMinus.35 = statesIP1TMinus.35,
                    statesIP3TMinus.35 = statesIP3TMinus.35,
                    statesIP1TMinus.55 = statesIP1TMinus.55, 
                    statesIP3TMinus.55 = statesIP3TMinus.55,
                    endstatesIP1TMinus.35 = endstatesIP1TMinus.35, 
                    endstatesIP3TMinus.35 = endstatesIP3TMinus.35,
                    endstatesIP1TMinus.55 = endstatesIP1TMinus.55, 
                    endstatesIP3TMinus.55 = endstatesIP3TMinus.55)
